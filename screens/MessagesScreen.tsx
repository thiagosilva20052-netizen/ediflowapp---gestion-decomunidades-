...
const MessagesScreen: React.FC<Props> = ({ navigate, role }) => {
                  ...
                  <div className="space-y-4">
                      {conciergeChat && (
                          <button 
                              ...
                              className="w-full bg-gradient-to-r from-[#1c262c] to-[#101c22] border border-ediflow-primary/20 p-5 rounded-3xl flex items-center gap-5 hover:bg-[#25323a] active:scale-[0.98] transition-all group"
                          >
                      ...
                      {adminChat && (
                          <button 
                              ...
                              className="w-full bg-gradient-to-r from-[#1c262c] to-[#101c22] border border-blue-500/20 p-5 rounded-3xl flex items-center gap-5 hover:bg-[#25323a] active:scale-[0.98] transition-all group"
                          >
              ...
              <div className="px-5 py-4">
                  <div className="relative">
                      ...
                      <input 
                          ...
                          className="w-full bg-[#1c262c] text-white rounded-xl py-3.5 pl-12 pr-4 text-sm focus:ring-1 focus:ring-ediflow-primary border border-white/5"
                      />
                  </div>
              </div>
                  ...
                  {filteredChats.length > 0 ? (
                      filteredChats.map(chat => (
                          <div 
                              ...
                              className="flex items-center gap-4 p-3 rounded-2xl bg-[#1c262c] border border-white/5 hover:bg-[#25323a] cursor-pointer active:scale-[0.98] transition-all"
                          >
                              <div className="relative">
                                  {chat.icon ? (
                                  ...
                                  {chat.online && <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 rounded-full border-[2.5px] border-[#1c262c]"></div>}
                              </div>
  ...
  if (activeChat) {
      ...
      return (
          <div className="flex flex-col min-h-full bg-[#101c22]">
              {/* Chat Header */}
              <div className="flex items-center gap-3 px-4 py-3 bg-[#1c262c] sticky top-0 z-30 border-b border-white/5 shadow-md">
                  <button 
                      onClick={handleBack}
                  ...
                  <div className="relative">
                      ...
                      {activeChat.online && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[#1c262c]"></div>}
                  </div>
                  <div className="flex-1 min-w-0">
              ...
              {/* Chat Messages */}
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#101c22]">
                      ...
                      <div key={msg.id} className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'}`}>
                          <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${
                              msg.isMe 
                                ...
                                : 'bg-[#25323a] text-white rounded-tl-sm border border-white/5'
                          }`}>
                              {msg.text}
                          </div>
              ...
              {quickReplies.length > 0 && (
                  <div className="px-2 py-2 bg-[#1c262c] border-t border-white/5 flex gap-2 overflow-x-auto no-scrollbar">
                      {quickReplies.map((reply, idx) => (
                          <button 
                              key={idx}
                              onClick={() => handleSendMessage(reply)}
                              className="whitespace-nowrap bg-[#25323a] hover:bg-[#2d3d46] text-xs text-gray-300 px-3 py-1.5 rounded-full border border-white/5 active:scale-95 transition-all"
                          >
                              {reply}
                          </button>
                      ))}
              ...
              {/* Chat Input */}
              <div className="p-3 bg-[#1c262c] border-t border-white/5 flex items-end gap-2">
                  ...
                  <div className="flex-1 bg-[#101c22] rounded-2xl border border-white/5 flex items-center pr-1 focus-within:border-ediflow-primary/50 transition-colors">
                      <textarea 
                          value={newMessage}
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#101c22]">
      {/* Header */}
      <div className="flex items-center justify-between px-5 pt-6 pb-4 bg-[#101c22] sticky top-0 z-30 border-b border-white/5">
