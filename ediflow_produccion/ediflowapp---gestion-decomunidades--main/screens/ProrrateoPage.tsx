...
const ProrrateoPage: React.FC<Props> = ({ navigate }) => {
  ...
  const handleEmit = async () => {
    ...
    try {
      ...
      // Persist the consolidated prorrateo result into `common_expenses` so reconciliation can match bank movements.
      const commonExpensesToInsert = prorrateoData.map(row => ({
        tenant_id: currentTenant.id,
        unit_id: row.unitId,
        period: billingMonth.charAt(0).toUpperCase() + billingMonth.slice(1),
        total_amount: Math.round((row.total + Number.EPSILON) * 100) / 100
      }));

      const { error: commonError } = await supabase
        .from('common_expenses')
        .upsert(commonExpensesToInsert, { onConflict: ['tenant_id', 'unit_id', 'period'] });

      if (commonError) {
        console.error('Error saving common_expenses:', commonError);
        throw commonError;
      }
